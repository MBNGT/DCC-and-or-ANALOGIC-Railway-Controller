...
/**********************************************************************
  ...
  Railway_Control Base Station is a C++ program written for the Arduino Uno and Arduino Mega
  ...
  This version of Railway_Control Base Station supports:
  ...
  Railway_Control Base Station is controlled with simple text commands received via
  ...
  Railway_Control CONTROLLER, available separately under a similar open-source
  ...
  via the Railway_Control Base Station.
  ...
  Neither Railway_Control Base Station nor Railway_Control CONTROLLER use any known proprietary or
  ...
  However, Railway_Control Base Station and Railway_Control CONTROLLER do heavily rely on the IDEs and
  ...
  Railway_Control to be developed and distributed in the same fashion.
  ...
  BRIEF NOTES ON THE THEORY AND OPERATION OF Railway_Control Base Station:

  Railway_Control Base Station for the Uno configures the OC0B interrupt pin associated with Timer 0,
  ...
  DCC timing standards.  When compiled for the Mega, Railway_Control Base Station uses OC3B instead of OC0B.
  ...
  registers numbered 1 through MAX_MAIN_REGISTERS (as defined in DCCpp_Mega.h).
  ...
  Railway_Control Base Station therefore sequentially loops through each main operations track packet regsiter
  ...
  Other Motor Shields may require different sets of jumper or configurations (see Config.h and DCCpp_Mega.h for details).
  ...
  Railway_Control Base Station in split into multiple modules, each with its own header file:

  DCCpp_Mega:        declares required global objects and contains initial Arduino setup()
  ...
  Railway_Control Base Station is configured through the Config.h file that contains all user-definable parameters
...
CurrentMonitor mainMonitor(CURRENT_MONITOR_PIN_MAIN, "<p20>");  // create monitor for current on Main Track
CurrentMonitor progMonitor(CURRENT_MONITOR_PIN_PROG, "<p30>");  // create monitor for current on Program Track
...
void setup() {
  ...
  Serial.print("<iRailway_Control Base station ");      // Print Status to Serial Line regardless of COMM_TYPE setting so use can open Serial Monitor and check configurtion
  ...
  // Initializing I2C
  int I2C_Device = 0;
  Wire.begin(); // set address #0 as a controller device
  I2C_Command::I2C_Scan(I2C_Device);
...
// Railway_Control Base Station was otherwise processing.  But once inside the interrupt, all other interrupt routines are temporarily diabled.
